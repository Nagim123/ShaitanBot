...
class PinterestScheduler(commands.Cog):
    ...
    async def add_pin_board(self, ctx: commands.Context, board_owner: str, board_name: str) -> None:
        ...
        if ctx.channel.id in self.channel_schedules:
            scheduler = PinterestBoardScheduler(board, f"{self.board_cache_folder_path}/{ctx.channel.id}/{board_owner}_{board_name}")
            self.channel_schedules[ctx.channel.id].append((f"{board_owner}_{board_name}", scheduler))
        else:
            os.mkdir(f"{self.board_cache_folder_path}/{ctx.channel.id}")
            scheduler = PinterestBoardScheduler(board, f"{self.board_cache_folder_path}/{ctx.channel.id}/{board_owner}_{board_name}")
    ...
    async def remove_pin_board(self, ctx: commands.Context, board_owner: str, board_name: str) -> None:
        if ctx.channel.id in self.channel_schedules:
            ...
            if os.path.exists(f"{self.board_cache_folder_path}/{ctx.channel.id}/{board_owner}_{board_name}"):
                scheduler_index = -1
                ...
                os.remove(f"{self.board_cache_folder_path}/{ctx.channel.id}/{board_owner}_{board_name}")
